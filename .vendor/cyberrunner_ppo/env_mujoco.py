...
# ============================================================================
# EASY MAZE LAYOUT  (ported verbatim from world-model-luca:cyberrunner_env_vision.py)
# ============================================================================
...
# ============================================================================
# MEDIUM MAZE LAYOUT  (ported verbatim from world-model-luca:cyberrunner_env_vision.py)
# ============================================================================
...
# Layout dispatch — used by env_mjx.CyberrunnerMJXEnv via maze_layout=...
_LAYOUT_LOADERS = {
...
def get_layout(name: str) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """Return (walls_h, walls_v, holes, waypoints) for the named maze layout."""
    if name not in _LAYOUT_LOADERS:
        raise ValueError(
            f"Unknown maze_layout {name!r}. "
            f"Expected one of {sorted(_LAYOUT_LOADERS)}."
        )
    return _LAYOUT_LOADERS[name]()
...
class CyberRunnerEnv(gym.Env):
    ...
    def __init__(
        ...
        # Frame-stacking config — mirrors env_mjx.py and cyberrunner_mjx
        # HistoryObserver. Each frame is [joint(2), ball(2), action(2)] = 6 dims;
        # current path-relative info (vec_to_closest, next_wp, next_next_wp) is
        # appended once at the end (not stacked).
        self.history_length = history_length
        self._frame_dim = 6
        self._obs_size = self.history_length * self._frame_dim + 6
        ...
        # Observation space (always a Dict). "states" carries the stacked obs.
        obs_spaces = {
            "states": spaces.Box(
        ...
        self._obs_history = np.zeros(
            (self.history_length, self._frame_dim), dtype=np.float32
        )
    ...
    def reset(
        ...
        # Initialise the history buffer by tiling the t=0 frame H times
        # (matches env_mjx.py + cyberrunner_mjx HistoryObserver reset).
        frame0, _ = self._build_frame(self._last_action)
        self._obs_history = np.tile(frame0[None, :], (self.history_length, 1))
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, bool, Dict[str, Any]]:
        ...
        # Roll the history buffer: drop oldest, append new frame.
        new_frame, _ = self._build_frame(self._last_action)
        self._obs_history = np.concatenate(
            [self._obs_history[1:], new_frame[None, :]], axis=0
        )
    ...
    def _build_frame(self, action: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        """Sample step noise, build a single 6-dim frame [joint, ball, action],
        and return `(frame, ball_pos_noisy)`. Caller stores `ball_pos_noisy` so
        path-relative obs uses the same noise sample as this frame.
        """
        ...
        frame = np.concatenate([joint_pos, ball_pos_noisy, action]).astype(np.float32)
        # Cache for `_get_obs` so the path-relative vectors share the same
        # noisy ball pos as the frame just appended to the history buffer.
        self._last_ball_pos_noisy = ball_pos_noisy.astype(np.float32)
        return frame, ball_pos_noisy.astype(np.float32)

    def _get_obs(self) -> Dict[str, np.ndarray]:
        """Build the stacked obs dict.

        Layout (mirrors env_mjx.py):
            history.flatten()  — last H frames of [joint(2), ball(2), action(2)]
            vec_to_closest(2)  — closest_point         - last_ball_pos_noisy
            vec_to_next_wp(2)  — waypoints[seg+1]      - last_ball_pos_noisy
            vec_to_next_next(2)— waypoints[seg+2]      - last_ball_pos_noisy
        Total: history_length * 6 + 6 dims.
        """
        ball_pos_noisy = self._last_ball_pos_noisy
