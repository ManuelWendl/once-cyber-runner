...
def main(argv=None):
  ...
  config = config.update({'env': {'cyberrunner': {
      'include_vision': config.vision,
      'randomize_init_pos': config.random_start,
...
def make_replay(config, folder, mode='train'):
  ...
  kwargs = dict(
      ...
      chunksize=config.replay.chunksize, directory=None)
