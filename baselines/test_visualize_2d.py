...
def draw_observation(ax, obs, env):
    ...
    joint_angles = obs[0:2]
    ball_pos = obs[2:4]
    vec_to_closest = obs[4:6]
    vec_to_next_wp = obs[6:8]
    vec_to_next_next_wp = obs[8:10]
...
def main():
    # Initialize environment (no rendering needed)
    env = CyberRunnerEnv(render_mode=None, randomize_init_pos=True)
