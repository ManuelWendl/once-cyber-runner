...
class Dreamer(nn.Module):
    def __init__(self, config, obs_space, act_space):
        ...
        # loss_scales may contain "ensemble" (OPAX, not ported); drop so totals match
        self._loss_scales.pop("ensemble", None)

        if self.rep_loss != "dreamer":
            raise NotImplementedError(
                f"Only rep_loss == 'dreamer' is supported in this port (got {self.rep_loss!r})."
            )
    ...
    def _cal_grad(self, data, initial):
        ...
        # === Representation loss: decoder reconstruction (plain DreamerV3) ===
        # (B, T, F)
