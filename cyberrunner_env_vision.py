...
# Per-component scales used to normalize the 10-dim state observation
# to roughly [-1, 1] before it is fed to the agent. Picked from physical
# bounds so symlog/MLP encoders see well-conditioned inputs.
_STATE_SCALES = np.array([
    RANGE_ALPHA[1],          # joint alpha
    RANGE_BETA[1],           # joint beta
    BOARD_WIDTH / 2,         # ball x
    BOARD_HEIGHT / 2,        # ball y
    BOARD_WIDTH / 2,         # vec ball -> closest path point (x)
    BOARD_HEIGHT / 2,        # vec ball -> closest path point (y)
    BOARD_WIDTH / 2,         # vec ball -> next waypoint (x)
    BOARD_HEIGHT / 2,        # vec ball -> next waypoint (y)
    BOARD_WIDTH / 2,         # vec ball -> next-next waypoint (x)
    BOARD_HEIGHT / 2,        # vec ball -> next-next waypoint (y)
], dtype=np.float32)
...
class CyberRunnerEnv(gym.Env):
    ...
    def __init__(
        ...
        if layout not in _LAYOUT_LOADERS:
            raise ValueError(f"Unknown layout {layout!r}; expected one of {list(_LAYOUT_LOADERS)}")
        ...
        # State is normalized to roughly [-1, 1] via _STATE_SCALES; we use
        # +/-2 here because ball->waypoint vectors can briefly exceed half
        # the board if the next waypoint is on the opposite side of the maze.
        obs_spaces = {
            "states": spaces.Box(low=-2.0, high=2.0, shape=(10,), dtype=np.float32),
        }
        if self.include_vision:
            self.img_size = 64
            n_channels = 1 if self.grayscale else 3
            obs_spaces["image"] = spaces.Box(0, 255, (self.img_size, self.img_size, n_channels), np.uint8)
    ...
    def _get_obs(self) -> Dict[str, np.ndarray]:
        ...
        if self.include_vision:
            ...
            img = self.vision_renderer.render()
            if self.grayscale:
                img = np.dot(img[..., :3], [0.299, 0.587, 0.114]).astype(np.uint8)[..., None]
            obs["image"] = img
