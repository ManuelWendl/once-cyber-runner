...
def build_model(
    ...
    floor_geom.rgba = [0.8, 0.8, 0.8, 1.0]
...
def _add_holes(board, holes: np.ndarray):
    """Add visual representation of holes as dark cylinders."""
    hole_rgba = [0.1, 0.1, 0.1, 1.0]  # Dark color to look like holes
    hole_height = 0.001  # Very thin cylinder
    z_pos = 0.0731  # Slightly above floor (0.073) so it's visible
    
    for i, hole in enumerate(holes):
        x, y = float(hole[0]), float(hole[1])
        
        hole_geom = board.add_geom()
        hole_geom.name = f"hole{i}"
        hole_geom.type = mujoco.mjtGeom.mjGEOM_CYLINDER
        hole_geom.pos = [x, y, z_pos]
        hole_geom.size = [HOLE_RADIUS, hole_height, 0]
        hole_geom.rgba = hole_rgba
        hole_geom.contype = 0  # No collision
        hole_geom.conaffinity = 0  # No collision
...
class CyberRunnerEnv(gym.Env):
    ...
    def __init__(
        ...
        # Observation: joints (2) + ball pos (2) + vec to closest path point (2) + vec to next 2 waypoints (4)
        obs_dim = 10
    ...
    def _get_obs(self) -> np.ndarray:
        ...
        # If closest point is on segment i (between waypoint i and i+1),
        # next waypoints are i+1 and i+2
...
if __name__ == "__main__":
    ...
    print(f"Initial obs: {obs[:4]}")  # joints + ball pos
